        ...
        function finish(err) {
            ...
            if (hash && !err) {
                ...
                if (md5Actual !== md5Expected) {
                    err = new errors.ChecksumError(md5Actual, md5Expected);
                }
            }
            callback(err);
        }
...
CLI.prototype.do_create = function do_create(subcmd, opts, args, callback) {
    ...
    function getManifestData(next) {
        if (opts.filename) {
            fs.readFile(opts.filename, 'utf8', next);
    ...
    getManifestData(function (err, data) {
        ...
        self.client.createImage(manifest, function (err, image, res) {
            ...
            if (err) {
                //XXX TODO: good error rendering of 'errors'
                return callback(self._errorFromClientError(err));
            }
            console.log(JSON.stringify(image, null, 2));
            callback();
...
CLI.prototype.do_create.description = (
    ...
    'activate the image, $NAME activate.\n' +
    '\n' +
    'Usage:\n' +
    '    $NAME create [-f MANIFEST-FILE]\n' +
    '    (manifest data on stdout) | $NAME create\n' +
    '\n' +
    'Options:\n' +
    '    -f MANIFEST-FILE   The manifest file with which to create\n'
);

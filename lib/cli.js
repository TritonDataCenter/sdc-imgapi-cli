...
function CLI(options) {
    ...
    // Load subcmds. Exclude the ones listed from the options
    var excludeCmds = options.excludeCmds || [];
    assert.arrayOfString(excludeCmds, 'options.excludeCmds');
    excludeCmds.forEach(function (cmd) {
        delete self.constructor.prototype['do_' + cmd];
    });
...
CLI.prototype.do_list = function do_list(subcmd, opts, args, callback) {
    ...
    this.client.listImages(filters, function (err, images, res) {
        ...
        sortArrayOfObjects(images, sortFields);
        if (opts.json) {
            ...
            images.forEach(function (i) {
                if (i.published_at) {
                    // Just the date.
                    i.published_date = i.published_at.slice(0, 10);
                    // Normalize on no milliseconds.
                    i.published = i.published_at.replace(/\.\d+Z$/, 'Z');
                }
                if (i.files && i.files[0]) {
                    i.size = i.files[0].size;
                }
                var flags = [];
                if (i.origin) flags.push('I');
                if (i['public']) flags.push('P');
                if (i.state !== 'active') flags.push('X');
                i.flags = flags.length ? flags.join('') : undefined;
            });
            try {
...
CLI.prototype.do_list.description = (
    ...
    '    flags              This is a set of single letter flags summarizing\n' +
    '                       some fields. "P" indicates the image is public.\n' +
    '                       "I" indicates an incremental image (i.e. has an\n' +
    '                       origin). "X" indicates an image with a state\n' +
    '                       *other* than "active".\n' +
    ...
    '                       "published_at,name".\n' +
    '    --latest           Only show the latest image, by published_at, for\n' +
    '                       a given (owner, name) set\n'
);
...
CLI.prototype.do_delete = function do_delete(subcmd, opts, args, callback) {
    ...
    this.client.deleteImage(uuid, options, function (err, res) {
...
CLI.prototype.do_delete.description = (
    ...
    '\n' +
    'Usage:\n' +
    '    $NAME delete UUID [OPTIONS]\n' +
    '\n' +
    'Options:\n' +
    '    -p, --purge When an IMGAPI instance is running on \'dc\' mode,\n' +
    '                image manifests are not deleted but marked as\n' +
    '                destroyed, only the image files are deleted. When\n' +
    '                \'purge\' is passed, the image manifest and its files\n' +
        ...
        ensureAddFileOpts(opts, function (fErr) {
            ...
            self.client.createImage(manifest, function (err, image, res) {
                ...
                if (!opts.file) {
                    ...
                    self.do_add_file('add-file', opts, [image.uuid], function (err2) {
                        if (err2) {
...
function parseKeyEqualsValue(args, multiple)
{
    ...
    for (arg in args) {
        kv = args[arg].split('=');
        if (kv.length < 2) {
            throw new TypeError('Bad arguments: unable to split ' + args[arg]);
        } else {
            key = kv[0];
            var objKeys = key.split('.');
            // Allow updating single attribute or simple object attributes
            if (objKeys.length > 1) {
                key = objKeys[0];
                val = {};
                var keyVal = kv.slice(1).join('=');
                // Parse into number for values that pass this test
                keyVal = (isNaN(Number(keyVal)) ? keyVal : Number(keyVal));
                val[objKeys.slice(1).join('')] = keyVal;
            } else {
                val = kv.slice(1).join('=');
            }
...
function mergeImageAttributes(body, image) {
    Object.keys(body).forEach(function (key){
        if (typeof (body[key]) === 'object' && image[key] !== undefined) {
            var orig = image[key];
...
CLI.prototype.do_update = function do_update(subcmd, opts, args, callback) {
    ...
    if (args.length > 1 && Object.keys(opts).length > 0) {
        return callback(new errors.UsageError(
            'Cannot provide options and property=value pairs at the same time'));
    }
    ...
    var method = (args.length > 1) ? getArgsData : getFileData;

    function getArgsData(image, next) {
        try {
            args.shift();
            var data = parseKeyEqualsValue(args);
            next(null, data);
        } catch (parseErr) {
            next(parseErr);
        }
    ...
    self.client.getImage(uuid, function (err, image, res) {
        ...
        method.call(self, image, function (err, data) {
            if (err) {
                return callback(err);
            }
            ...
            self.client.updateImage(uuid, body, function (err, image, res) {
                self.log.trace({err: err, image: image, res: res}, 'UpdateImage');
                if (err) {
                    return callback(self._errorFromClientError(err));
                }
                console.log('Update image %s (%s, %s, state=%s)', image.uuid,
                    image.name, image.version, image.state);
                callback();
            });
        });
...
CLI.prototype.do_update.description = (
    ...
    'nic_driver, disk_driver, cpu_type, image_size and inherited_directories.\n' +
    '\n' +
    'Usage:\n' +
    ...
    '\n' +
    'Options:\n' +
    '    -f JSON-FILE       JSON file containing the properties/values of the\n' +
    '                       image that need to be updated\n'
);
...
CLI.prototype.do_import = function do_import(subcmd, opts, args, callback) {
    ...
    getManifestData(function (err, data) {
        ...
        ensureAddFileOpts(opts, function (fErr) {
            ...
            self.client.adminImportImage(manifest, importOpts,
                                         function (err, image, res) {
...
CLI.prototype.do_import.description = (
    ...
    '"published_at" fields are preserved. This is an operator-only action used\n' +
    'for transferring images between IMGAPI servers.\n' +
    '\n' +
    'An image may be imported directly from another IMGAPI by using the "-S"\n' +
    'option. In this case the target IMGAPI will handle retrieving the\n' +
    ...
    '\n' +
    'Alternatively, a local manifest file may be imported with the "-m" option\n' +
    'or on stdin. The resulting image is *unactivated*. The typical next steps\n' +
    'are to add the image file ($NAME add-file) then activate the image\n' +
    '($NAME activate). All three steps can be done in one by specifying\n' +
    'the "-f FILE" option.\n' +
    '\n' +
    'Usage:\n' +
    ...
    '\n' +
    'Options:\n' +
    '    -S SOURCE-URL      Source URL of the remote IMGAPI repository. This is\n' +
    '                       the full URL to the origin IMGAPI without any suffix\n' +
    '                       endpoint or action. Example: https://images.joyent.com\n' +
    '\n' +
    ...
    '                       file. One of "gzip", "bzip2" or "none". If not\n' +
    '                       given, it is inferred from the file extension.\n' +
    '                       This option is not allowed for *source* URL imports.\n' +
    '    -s SHA1            SHA-1 hash of the image file. If given, the\n' +
    '                       server will use it compare it with the uploaded\n' +
    '                       file SHA-1. This option is not allowed for *source*\n' +
    '                       URL imports.\n' +
    '    -q, --quiet        Disable upload progress bar.\n' +
    '\n' +
    '    --skip-owner-check Skip the check that the "owner" UUID exists in\n' +
    '                       the user database. This check is only done for\n' +
    '                       IMGAPI instances inside an SDC. I.e. this option\n' +
    '                       does not apply to images.joyent.com.\n'
);
...
CLI.prototype.do_add_icon = function do_add_file(subcmd, opts, args, callback) {
    var self = this;
    if (args.length !== 1) {
        return callback(new errors.UsageError(format(
            'incorrect number of args (%d): %s', args.length, args.join(' '))));
...
CLI.prototype.do_add_icon.description = (
    'Add an image icon.\n' +
    '\n' +
    'An optional feature to add an icon file to an image. Icon files must not\n' +
    'be bigger than 128KB and they must be one of the following file types:\n' +
    '.jpg, .png or .gif.\n' +
    '\n' +
    'Usage:\n' +
    ...
    'Options:\n' +
    '    -f FILE            Image file to add\n' +
    '    -c CONTENT_TYPE    Content type of the icon file. One of "jpeg" (or jpg),\n' +
    '                       "png" or "gif". If not given, it may be inferred\n' +
    '                       from common file extensions.\n' +
    '    -s SHA1            SHA-1 hash of the image icon file. If given, the\n' +
    '                       server will use it compare it with the uploaded\n' +
    '                       file SHA-1\n' +
    '    -q, --quiet        Disable progress bar.\n'
);
...
CLI.prototype.do_enable.description = (
    ...
    'Note that if the image is not yet activated, the state will remain\n' +
    '"unactivated". In SDC a disabled image is not available for provisioning\n' +
    '\n' +
    'Usage:\n' +
    '    $NAME enable UUID\n'
);
...
CLI.prototype.do_add_acl.description = (
    ...
    '\n' +
    'Usage:\n' +
    '    $NAME add-acl UUID account-uuid1 [ account-uuid2 [ account-uuid3 ... ] ]\n'
);


CLI.prototype.do_remove_acl = function do_add_acl(subcmd, opts, args, callback) {
    var self = this;
...
CLI.prototype.do_remove_acl.description = (
    ...
    '\n' +
    'Usage:\n' +
    '    $NAME remove-acl UUID account-uuid1 [ account-uuid2 [ account-uuid3 ... ] ]\n'
);

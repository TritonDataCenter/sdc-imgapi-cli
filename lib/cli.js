 ...
 *      - @param name {String} Required. The CLI name, e.g. "images-imgadm".
...
CLI.prototype.dispatch = function dispatch(subcmd, argv, callback) {
    ...
    //      `images-imgadm -u trentm -j images`
    ...
    this.client.listImages(filters, listOpts, function (err, images, res) {
        ...
        if (opts.json) {
            ...
            if (opts.tree) {
                ...
                // First pass: enable image lookup by UUID.
                for (i = 0; i < images.length; i++) {
                    img = images[i];
                    imageFromUuid[img.uuid] = img;
                }

                // Second pass: initialize a `_treeStack` for each image.
                //
                // The complete `_treeStack` is an ancestry array of image UUIDs
                // indicating an image's position in the origin hierarchy, e.g.:
                //
                //  imageA._treeStack = [
                //      <image A's top-level non-incremental image UUID>,
                //      <image A's grandparent image UUID>,
                //      <image A's parent image UUID>
                //  ]
                for (i = 0; i < images.length; i++) {
                    img = images[i];
                    if (img.origin) {
                        ...
                        if (!imageFromUuid[img.origin]) {
                            ...
                            columns.forEach(function onCol(col) {
                                if (!faux[col]) {
                                    faux[col] = '???';
                                }
                            });
                ...
                // Third+ passes: Each pass works through another level of
                // parentage until each image reaches its top-level
                // non-incremental ancestor.
                while (remainingImgs.length) {
                    var stillRemainingImgs = [];
                    for (i = 0; i < remainingImgs.length; i++) {
                        img = remainingImgs[i];
                        var originUuid = img._treeStack[0];
                        var originImg = imageFromUuid[originUuid];
                        if (originImg.origin) {
                            img._treeStack.unshift(originImg.origin);
                            stillRemainingImgs.push(img);
                        } else {
                            img._treeStack = imageFromUuid[originUuid].
                                _treeStack.concat(img._treeStack);
                        }
                    }
                    remainingImgs = stillRemainingImgs;
                }
...
CLI.prototype.do_list.description = (
    ...
    '<https://github.com/TritonDataCenter/sdc-imgapi/blob/master/docs/index.md#listimages-get-images>.\n' +
    '\n' +
    'Usage:\n' +
...
CLI.prototype.do_create.description = (
    ...
    '    -c COMPRESSION     Specify the compression used for the image file.\n'+
    '                       One of "gzip", "bzip2", "xz" or "none". If not\n' +
...
CLI.prototype.do_import = function do_import(subcmd, opts, args, callback) {
    ...
    if (args.length > 1) {
        ...
        // `*-imgadm import -S <source-url> [-C channel] <uuid>`
        uuid = args[0];
        assert.uuid(uuid, 'uuid');
        ...
        try {
            purl = url.parse(opts['source-url'], true);
        } catch (syntaxErr) {
            return callback(new errors.UsageError(format(
                'invalid URL "%s"', opts['source-url'])));
        }

        /*
         * Prior to supporting -C, users would typical specify the channel via a
         * query param in the URL, so we continue to support this.
         */
        if (opts['channel'] === undefined) {
            if (purl.query['channel'] === undefined) {
                purl.query['channel'] = '*';
            }
        } else {
            if (purl.query['channel'] === undefined) {
                purl.query['channel'] = opts['channel'];
            } else {
                return (callback(new errors.UsageError('channel specified ' +
                    'twice')));
            }
        }
        ...
        self.client.adminImportRemoteImageAndWait(uuid, url.format(purl),
            importOpts, function (err, image, res) {
            rollbackImage = image;
...
CLI.prototype.do_import.description = (
    ...
    '                       without any suffix endpoint or action, e.g.:\n' +
    '                       https://images.smartos.org\n' +
    '                       https://updates.tritondatacenter.com\n' +
    '    -C channel         Import the image UUID from the given channel.\n' +
    '                       If not specified, all channels are examined.\n' +
    '\n' +
    ...
    '                       IMGAPI instances inside Triton. I.e. this\n' +
    '                       option does not apply to images.smartos.org.\n'
);

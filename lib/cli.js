        ...
        if (opts.json) {
            ...
            try {
                ...
                for (var j = 0; j < images.length; j++) {
                    if (images[j].channels) {
                        images[j].channels = images[j].channels.join(',');
                    }
                }
                tabula(images, {
...
CLI.prototype.do_delete = function do_delete(subcmd, opts, args, callback) {
    ...
    this.client.deleteImage(uuid, delOpts, function (err, res) {
        ...
        if (!self.features.channels || delOpts.forceAllChannels) {
            ...
            console.log('Deleted image %s from channel "%s"',
...
CLI.prototype.do_update = function do_update(subcmd, opts, args, callback) {
    ...
    self.client.getImage(uuid, function (err, image, res) {
        ...
        getData.call(self, image, function (err, data) {
            if (err) {
                return callback(err);
            }
...
CLI.prototype.do_import.description = (
    ...
    '\n' +
    'Options:\n' +
    '    -S SOURCE-URL      URL of the remote IMGAPI repository source. This is\n' +
    '                       the full URL to the origin IMGAPI without any suffix\n' +
    '                       endpoint or action. Example: https://images.joyent.com\n' +
    '                       To import from a particular source *channel*, append\n' +
...
CLI.prototype.do_add_icon.description = (
    ...
    'Options:\n' +
    '    -f FILE            Image file to add\n' +
    '    -c CONTENT_TYPE    Content type of the icon file. One of "image/jpeg" (or jpg),\n' +
    '                       "image/png" or "image/gif". If not given, it may be inferred\n' +
        ...
        func: function channelAddOne(uuid, next) {
            ...
            self.client.channelAddImage(chOpts, function (err, img, res, req) {
                ...
                if (err) {
                    ...
                    console.error('Error adding image %s to channel "%s"',
                        uuid, channel);
                } else {
                    console.log('Added image %s to channel "%s"',
                        uuid, channel);
                }
                next();
            });
